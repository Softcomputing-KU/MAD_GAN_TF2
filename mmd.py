...
def _mix_rbf_kernel(X, Y, sigmas, wts=None):
    ...
    return K_XX, K_XY, K_YY, tf.reduce_sum(input_tensor=wts)
...
def _mmd2(K_XX, K_XY, K_YY, const_diagonal=False, biased=False):
    ...
    if biased:
        mmd2 = (tf.reduce_sum(input_tensor=K_XX) / (m * m)
              + tf.reduce_sum(input_tensor=K_YY) / (n * n)
              - 2 * tf.reduce_sum(input_tensor=K_XY) / (m * n))
    else:
        ...
        mmd2 = ((tf.reduce_sum(input_tensor=K_XX) - trace_X) / (m * (m - 1))
              + (tf.reduce_sum(input_tensor=K_YY) - trace_Y) / (n * (n - 1))
              - 2 * tf.reduce_sum(input_tensor=K_XY) / (m * n))
...
def _mmd2_and_variance(K_XX, K_XY, K_YY, const_diagonal=False, biased=False):
    ...
    if const_diagonal is not False:
        ...
        diag_X = tf.linalg.tensor_diag_part(K_XX)
        diag_Y = tf.linalg.tensor_diag_part(K_YY)

        sum_diag_X = tf.reduce_sum(input_tensor=diag_X)
        sum_diag_Y = tf.reduce_sum(input_tensor=diag_Y)

        sum_diag2_X = sq_sum(diag_X)
        sum_diag2_Y = sq_sum(diag_Y)

    Kt_XX_sums = tf.reduce_sum(input_tensor=K_XX, axis=1) - diag_X
    Kt_YY_sums = tf.reduce_sum(input_tensor=K_YY, axis=1) - diag_Y
    K_XY_sums_0 = tf.reduce_sum(input_tensor=K_XY, axis=0)
    K_XY_sums_1 = tf.reduce_sum(input_tensor=K_XY, axis=1)

    Kt_XX_sum = tf.reduce_sum(input_tensor=Kt_XX_sums)
    Kt_YY_sum = tf.reduce_sum(input_tensor=Kt_YY_sums)
    K_XY_sum = tf.reduce_sum(input_tensor=K_XY_sums_0)
